...
<body>
		...
		<div class="content-area">
			...
			<?php if ($view_mode === 'events'): ?>
				...
				<div class="events-grid">
					...
					?>
						<!-- event card (use explicit button to open volunteer applications) -->
	...
	<script>
		...
		function showEventVolunteers(eventId) {
			...
			// endpoint is in the same admin folder
			const volunteersUrl = 'get_event_volunteers.php?event_id=' + encodeURIComponent(eventId);
			...
			fetch(volunteersUrl)
				.then(res => {
					...
					if (!ctype.includes('application/json')) {
						return res.text().then(text => {
							throw new Error('Expected JSON but received: ' + (text ? text.slice(0,200) : '[empty response]'));
						});
					}
					return res.json();
				})
				.then(data => {
					...
					if (approved.length === 0) {
						...
						approved.forEach(v => {
							const profileLink = 'community_service.php?view=history&resident_id=' + encodeURIComponent(v.resident_id);
					...
					// Pending: plain name + Approve / Reject buttons
		...
		function showVolunteerApplications(eventId) {
			...
			// endpoint is in the same admin folder
			const appsUrl = 'get_volunteer_applications.php?event_id=' + encodeURIComponent(eventId);
